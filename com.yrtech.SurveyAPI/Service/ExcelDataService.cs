...
namespace com.yrtech.SurveyAPI.Service
{
    public class ExcelDataService
    {
        ...
        #region 自检
        // 导入日期
        public List<ProjectDto> TaskProjectDateImport(string ossPath)
        {
            // 从OSS下载文件
            string downLoadFilePath = basePath + @"Excel\ExcelImport\" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".xlsx";
            OSSClientHelper.GetObject(ossPath, downLoadFilePath);
            ...
            for (int i = 0; i < 10000; i++)
            {
                string date = sheet.GetCell("A" + (i + 3)).Value == null ? "" : sheet.GetCell("A" + (i + 3)).Value.ToString().Trim();
        ...
        // 导入经销商
        public List<ShopDto> TaskProjectShopImport(string ossPath)
        {
            // 从OSS下载文件
            string downLoadFilePath = basePath + @"Excel\ExcelImport\" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".xlsx";
            OSSClientHelper.GetObject(ossPath, downLoadFilePath);
        ...
        public List<FileResultDto> SubjectAndFileImport(string ossPath)
        {
            ...
            for (int i = 0; i < 10000; i++)
            {
                ...
                subject.Date = date;
                subject.SubjectCode = sheet.GetCell("B" + (i + 3)).Value == null ? "" : sheet.GetCell("B" + (i + 3)).Value.ToString().Trim();
                string orderNO = sheet.GetCell("C" + (i + 3)).Value.ToString();
                ...
                {
                    subject.OrderNO = Convert.ToInt32(sheet.GetCell("C" + (i + 3)).Value.ToString().Trim());
                }
                ...
                //}
                subject.Implementation = sheet.GetCell("D" + (i + 3)).Value == null ? "" : sheet.GetCell("D" + (i + 3)).Value.ToString().Trim();
                ...
                subject.ImproveAdvice = sheet.GetCell("J" + (i + 3)).Value == null ? "" : sheet.GetCell("J" + (i + 3)).Value.ToString().Trim();
